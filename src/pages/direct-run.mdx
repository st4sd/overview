...
For example, you can run the toy workflow [`nanopore-geometry-experiment`](https://github.com/st4sd/nanopore-geometry-experiment) like so:
...
The experiment should take about 5 minutes to complete. The `-l 40` option keeps the log printouts to the bare minimum. When the experiment completes expect to see something similar to the following text on your terminal:
...
Running an experiment creates a directory which contains the outputs. For example, the code above creates the directory `nanopore-geometry-experiment.instance`. See Section [What is the output of my experiment?](#what-is-the-output-of-my-experiment) for more information.
...
All outputs of the experiment are placed in a directory called `${package-name}-${timestamp}.instance` which is created in the directory you were in when you ran `elaunch.py`. The structure of this directory is:

* `output`: contains the runtime logs and files describing the outputs and status of your experiment
* `stages`: contains one directory per stage of your experiment. Each stage directory contains one directory per component in that stage of your experiment.
...
The key directories are `output` and `stages`.

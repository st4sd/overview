...
#### Where can I see what steps were executed during the experiment?
The **`stages`** directory holds the working directories for each step that was executed during this experiment instance. It’s useful for debugging or understanding the experiment’s workflow.

#### Where can I find the properties that my experiment measured ?
The **`output`** directory contains the measured properties of the input systems that were processed. It also contains metadata about the key outputs of your experiment.<br/>
🔗 [Learn more about the output directory](/direct-run-advanced#the-output-directory)

#### Where does the experiment get information about nanoporous materials?
The **`nanopore-database`** directory is an application dependency used by the experiment to read data about the nanoporous materials it simulates.<br/>
🔗 [Learn more about Application Dependencies](/workflow-specification#application-dependencies)

#### Where can I find the input files and parameters used in the experiment?
The **`input`** directory contains the input files and variable parameterization files that were provided to the `elaunch.py` command.

#### Where is the experiment definition defined?
The **`conf`** directory contains the experiment definition that describes how the steps of the experiment should run.

#### Where is the code that generates interface properties or checks task completion?
The **`hooks`** directory contains code used to produce interface properties. It may also include scripts for verifying task completion, although this feature is not used in the `nanopore-geometry-experiment`.

...
<AnchorLink>Connecting to ST4SD</AnchorLink>
 <AnchorLink>Adding a virtual experiment package</AnchorLink>
  ...
  <AnchorLink>Common Recipes and Examples</AnchorLink>
</AnchorLinks>
...
**Important Size Limit**: The combined size of all `content` fields in `inputs` and `data` must be less than **1 MB**. For larger files, use S3, Datashim datasets, or PersistentVolumeClaims instead.
...
* data: (array of objects) each object specifies an override for one data file. See parmeterisation notes. Similar to "input" files, "data" files support S3, Datashim, and Volumes. See notes.
  - filename: str # required
    content: str # optional, mutually exclusive with volume - see S3 and Volume notes
    volume: str  # optional, mutually exclusive with content - see Volume notes
* volumes: (array of objects) This is an optional field defining volumes to mount
  ...
  * applicationDependency: (str) optional, name of the application dependency to bind to the volume. See the [Application Dependencies](/workflow-specification#application-dependencies) docs for more information.
  * subPath: (str) optional, path inside volume to mount (defaults to root of object). Applies only to persistentVolumeClaim and dataset types
  * readOnly: (bool) optional, whether to mount the volume as read-only (defaults to true)
  * identifier: (str) optional, a unique identifier for this volume that can be referenced in input/data file specifications via the `volume` field
...
1. (Volumes) `inputs` and `data` file-specifications can be fed from files that exist in a volume (PVC, ConfigMap, Secret). This is an alternative to setting the value of the fields directly in the payload or loading the contents of the file from a S3 bucket or Datashim Dataset. When using volumes, you must:
   - Define the volume in the `volumes` array with a unique `identifier`
   - Reference that `identifier` in the `volume` field of your input/data file specification
   - Specify the path to the file within the volume using the `filename` field
...
This section provides ready-to-use code snippets and examples for common scenarios when starting experiments programmatically.

### Recipe 1: Start experiment with inline input content

Use this when you have small input data in memory (e.g., from a pandas DataFrame or string):
...
rest_uid = api.api_experiment_start("my-experiment", payload=payload)
print(f"Started experiment: {rest_uid}")
```

<InlineNotification kind="warning">

Remember: The combined size of all `content` fields must be less than 1 MB. For larger files, use PVC, S3, or Datashim (see recipes below).

</InlineNotification>

### Recipe 2: Start experiment with inputs from PVC

Use this when your input files are already stored in a PersistentVolumeClaim:
...
### Recipe 3: Start experiment with S3 inputs and auto-upload outputs

Use this when inputs are in S3 and you want outputs automatically uploaded back to S3:
...
payload = {
    ...
    "s3Store": {
        "bucketPath": "/experiment-outputs/run-001/",
        "credentials": {
            "accessKeyID": "YOUR_ACCESS_KEY",
            "secretAccessKey": "YOUR_SECRET_KEY",
...
### Recipe 4: Start experiment and wait for completion

Use this to start an experiment and wait for it to finish:
...
while True:
    status = api.api_rest_uid_status(rest_uid)
    state = status['status']['experiment-state']
    exit_status = status['status'].get('exit-status', 'N/A')
    
    print(f"State: {state}, Exit Status: {exit_status}")
    ...
    if state == 'finished':
        if exit_status == 'Success':
            ...
            print(f"Experiment failed: {status['status'].get('error-description', 'Unknown error')}")
            break
    elif state == 'failed':
        print(f"Experiment failed to start: {status['status'].get('error-description', 'Unknown error')}")
        break
    
    time.sleep(10)  # Wait 10 seconds before checking again
```

### Recipe 5: Start experiment with custom metadata and environment variables

Use this to add custom tracking metadata and inject environment variables:
...
status = api.api_rest_uid_status(rest_uid)
user_metadata = status['status']['meta'].get('userMetadata', {})
print(f"Experiment metadata: {user_metadata}")
```

### Recipe 6: Start experiment with multiple volumes (PVC + ConfigMap)

Use this when you need to mount both data from a PVC and configuration from a ConfigMap:
...
payload = {
    ...
    "volumes": [
        {
            "identifier": "data-volume",
            "type": {
                "persistentVolumeClaim": "training-data-pvc"
            },
            "readOnly": True
        },
        {
            "identifier": "config-volume",
            "type": {
                "configMap": "model-configs"
            },
            "readOnly": True
        }
    ],
...
rest_uid = api.api_experiment_start("my-ml-experiment", payload=payload)
print(f"Started experiment: {rest_uid}")
```

#### Recipe 7: Wait for datastore consistency after experiment completion

After an experiment completes, there may be a delay before the experiment document is fully registered in the datastore. This recipe shows how to poll for the experiment document to ensure it's available before attempting to access its data.

```python
import experiment.service.db
import time

# Assume you already have the rest_uid from Recipe 4
# rest_uid = api.api_experiment_start(...)

# Wait for experiment to complete (see Recipe 4)
# ...

# Get the instance ID from the datastore using the rest_uid
user_metadata = api.cdb_get_document_user_metadata_for_rest_uid(rest_uid)
instance = user_metadata['instance']
print(f"Experiment instance: {instance}")

# Poll for experiment document availability in datastore
...
while True:
    try:
        # Attempt to retrieve the experiment document from datastore
        exp_docs = api.cdb_get_document_experiment(instance=instance)
        
        if exp_docs:
            exp_doc = exp_docs[0]
            doc_status = exp_doc.get('status', {})
            experiment_state = doc_status.get('experiment-state', 'unknown')
            
            print(f"Datastore status: {experiment_state}")
            
            # Check if experiment has reached a terminal state in datastore
            if experiment_state.lower() in ['finished', 'failed']:
                print(f"Experiment document is consistent in datastore")
                break
        
        elapsed = time.time() - start_time
        if elapsed >= max_wait_seconds:
            print(f"Timeout: Experiment document not consistent after {max_wait_seconds} seconds")
            break
            
        print(f"Waiting for datastore consistency... ({elapsed:.0f}s elapsed)")
        time.sleep(poll_interval_seconds)
        
    except Exception as e:
        elapsed = time.time() - start_time
        if elapsed >= max_wait_seconds:
            print(f"Timeout: Error accessing datastore after {max_wait_seconds} seconds: {e}")
            raise
        print(f"Waiting for datastore... ({elapsed:.0f}s elapsed)")
        time.sleep(poll_interval_seconds)

# Now you can safely access experiment data from datastore
print(f"Experiment exit status: {exp_doc.get('status', {}).get('exit-status', 'N/A')}")
```

### Providing inputs from a PersistentVolumeClaim (PVC)

You can provide input files from a PersistentVolumeClaim (PVC) that exists in your OpenShift namespace. This is useful when you have large input files or datasets already stored in persistent storage.

To use a PVC for input files:

1. Define the volume in the `volumes` array with a unique `identifier`
2. Reference that identifier in the `volume` field of your input file specification
3. Specify the path to the file within the PVC using the `filename` field

Example: Using a PVC named `my-data-pvc` that contains input files

```python
payload = {
    "inputs": [{
        "filename": "data/input_file.csv",
        "volume": "my-inputs"
    }],
    "volumes": [{
        "identifier": "my-inputs",
        "type": {
            "persistentVolumeClaim": "my-data-pvc"
        },
        "subPath": "experiment-inputs",
...
In this example:
- The PVC `my-data-pvc` is mounted with the identifier `my-inputs`
- The `subPath` field specifies that only the `experiment-inputs` subdirectory within the PVC is mounted
- The input file is read from `data/input_file.csv` within that mounted subdirectory
- The volume is mounted as read-only for safety

You can also mount the entire PVC by omitting the `subPath` field:
...
Multiple input and data files can reference the same volume identifier, allowing you to provide multiple files from a single PVC using only one volume entry.
